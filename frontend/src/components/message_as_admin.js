...
const Message_other = () => {
    ...
    const handleAddMessage = async () => {
        ...
        updatedComplaint.message.push({
            Data: newMessage,
            Identify: "Admin" // You can replace this with the actual sender's name
        });
        ...
        try {
            ...
            usersSnapshot.forEach(async (userDoc) => {
                ...
                const query1 = where("complaint_id", "==", localStorage.getItem('complaint_id_message'));
    ...
    //           });
    //           const assignedComplaints = allComplaints.filter(complaint => complaint.complaint_id === localStorage.getItem('complaint_id_message'));
            ...
            <div className="max-w-7xl w-full bg-white p-8 rounded-lg shadow-lg">
                ...
                <div className="overflow-y-auto h-96">
                    {complaint.message.map((msg, index) => (
                         ...
                         {msg.Identify === 'Admin' ? (
                             <div className={`bg-blue-200 p-3 rounded-lg mb-4 max-w-md ${msg.Identify === 'Admin' ? 'ml-auto' : 'mr-auto'}`}>
                                 <p className="text-base text-gray-800">{msg.Data}</p>
                                 <p className="text-sm text-gray-600">Sent By: {msg.Identify}</p>
                             </div>
                         ) : (
                             <div className={`bg-yellow-200 p-3 rounded-lg mb-4 max-w-md ${msg.Identify === 'Victim' ? 'mr-auto' : 'ml-auto'}`}>
                                 <p className="text-base text-gray-800">{msg.Data}</p>
                                 <p className="text-sm text-gray-600">Sent By: {msg.Identify}</p>
                             </div>
                         )}
